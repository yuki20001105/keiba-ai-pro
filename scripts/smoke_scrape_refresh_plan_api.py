#!/usr/bin/env python3
"""Smoke test for /api/scrape/refresh-plan route (dry-run preview only)."""
...
OUTPUT_PATH = Path("reports") / "scrape_refresh_plan_api_smoke_result.json"
TIMEOUT_SECONDS = 30
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke test for /api/scrape/refresh-plan")
    ...
    endpoint = f"{base}/api/scrape/refresh-plan"
    ...
    try:
        # unauth behavior
        unauth_status, unauth_body, _ = _http_json("POST", endpoint, payload={"target": "all", "policy": "dry-run"}, token=None)
        ...
        # valid dry-run request
        payload = {
            "target": "all",
            "policy": "repair-missing",
            "staleDays": 30,
            "currentParserVersion": "2.0.0",
        }
        status_ok, body_ok, json_ok = _http_json("POST", endpoint, payload=payload, token=token)
        plan = json_ok.get("plan") if isinstance(json_ok.get("plan"), dict) else {}
        dry_ok = bool(json_ok.get("dry_run") is True)
        has_counts = isinstance(plan.get("skip_count"), int) and isinstance(plan.get("repair_count"), int)
        update_disabled = bool(json_ok.get("update_enabled") is False and str(json_ok.get("update_action")) == "not-implemented")

        result["checks"]["dry_run_response"] = {
            "result": "pass" if (status_ok == 200 and dry_ok and has_counts and update_disabled) else "fail",
            "reason": "ok" if (status_ok == 200 and dry_ok and has_counts and update_disabled) else "invalid-response",
            "status": status_ok,
        }

        # forbidden path input rejection
        status_path, _, json_path = _http_json(
            "POST",
            endpoint,
            payload={"target": "all", "policy": "dry-run", "dbPath": "C:/tmp/db.sqlite"},
            token=token,
        )
        ...
        # update action endpoint disabled
