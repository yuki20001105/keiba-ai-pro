...
# 2) API Operations section visible
print('s2_api_operations_visible', 'API Operations' in h1)

# 3) GET curl snippets visible
print('s3_get_dashboard_curl', 'curl -sS "${API_BASE_URL:-http://127.0.0.1:8000}/api/mlops/research/scenario-router/ops/dashboard?' in h1)
print('s3_get_alerts_curl', '/api/mlops/research/scenario-router/alerts?' in h1)

# 4) target/limit reflected in snippets
...
# 5) POST/execute/rollback/stop curl snippets are not rendered
forbidden = ['curl -X POST', '/execute', '/resolve', '/rollback', '/stop-canary']
print('s5_no_forbidden_snippets', not any(x in h1.lower() for x in [f.lower() for f in forbidden]))

# 6) empty alerts/incidents still stable
