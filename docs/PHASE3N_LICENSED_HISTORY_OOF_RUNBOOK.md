...
The completed 2020-2024 folds are now a fixed, observed ability benchmark.
Do not select new features, model parameters, probability calibration, or
wagering thresholds by repeatedly inspecting those outcomes. Any later ability
claim needs a newly reserved chronological period.
...
The Phase3N value evaluator accepts only complete `middle` snapshots whose
quote timestamp is strictly before race start and no more than 30 minutes old.
Every row must carry a non-empty source and one shared, non-zero 40-character
candidate commit SHA. `yoso`, `final`, `result`, stale, incomplete, mixed-SHA,
or timestamp-free input fails closed before model fitting or strategy search.
The tracked search contract limits the realized candidate bet rate to 5-15%
and one wager per race.
...
The canonical evaluator columns are `post_time`, `odds_observed_at`,
`odds_source`, `odds_snapshot_kind`, `odds_status`, and
`candidate_commit_sha`. The explicit evidence aliases `race_start_at`,
`quote_at`, and `exact_commit_sha` are also accepted. `odds_status` must be
exactly `middle`; an allowed pre-race snapshot kind does not relax that rule.
...
The winner meta-model uses only OOF speed scores and market-free race-relative
derivatives. Meta-level OOF folds are again expanding-year
splits. The timestamped odds enter only after probability inference, in the
EV/market-gap strategy layer. EV threshold,
...
Feasibility requires at least 100 inner wagers, realized bet rate 5-15%,
maximum drawdown at most 20%, at least a one-point ROI advantage over the
approved lowest-valid-win-odds baseline, multi-year coverage, and no more than
one wager in any race. Feasible candidates are ranked first by the lower bound
of the 95% ROI confidence interval, then drawdown, baseline delta, year-level
stability, and adjacent-grid robustness. The report preserves all tested
conditions and rejection reasons, yearly metrics, confidence intervals,
approved-policy/search/input digests, source/timestamp ranges, and exact SHA.
If no feasible condition exists, evaluation ends with no selected policy.
