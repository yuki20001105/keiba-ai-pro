...
1. `Ctrl+R` でブラウザをリロードし、`/home` を開く
2. データ取得、モデル作成、予測実行、成績確認、ユーザー管理が表示されることを確認
3. Admin専用機能を開き、現在のパスワードで本人確認できることを確認

画面モードの切り替えはありません。本人確認は管理操作を初めて開く時だけ行います。一般ユーザーには予測実行と成績確認だけを表示し、旧 `/admin` URL は `/home` へリダイレクトされます。
