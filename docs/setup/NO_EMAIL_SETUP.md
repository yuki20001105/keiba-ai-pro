...
1. `/home` ページに5つの機能が自動表示される
2. Admin専用機能を開き、現在のパスワードで本人確認
3. ホームから以下にアクセス可能:
...
管理者／ユーザー画面の切り替え操作はありません。管理操作を初めて開く時だけ本人確認します。一般ユーザーには予測実行と成績確認だけを表示し、旧 `/admin` URL は `/home` へリダイレクトされます。
