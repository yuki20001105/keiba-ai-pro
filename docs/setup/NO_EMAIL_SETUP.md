...
1. `/home` ページのヘッダーに「管理者モード」が表示される
2. 「管理者モード」をクリックし、現在ログイン中の管理者アカウントのパスワードを再入力
3. 同じ `/home` 内で管理者ワークスペースへ切り替わることを確認
4. 管理者ワークスペースから以下にアクセス可能:
   - `/data-collection` - データ収集
   - `/train` - モデル学習
   - `/production-readiness` - 本番前チェック

一般ユーザーには管理者用の切り替え操作と内容は表示されません。旧 `/admin` URL はロックされた `/home` へリダイレクトされます。
