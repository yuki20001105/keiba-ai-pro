...
1. **ブラウザをリロード** (Ctrl+R)
2. `/home` に5つの機能が自動表示されることを確認
3. Admin専用機能を開き、現在のパスワードで本人確認
4. ホームから以下の機能へアクセス:
...
画面モードの切り替えはありません。管理操作を初めて開く時だけ本人確認し、その後15分間は再入力不要です。一般ユーザーには予測実行と成績確認だけを表示します。旧 `/admin` URL は管理画面を表示せず、`/home` へリダイレクトされます。
