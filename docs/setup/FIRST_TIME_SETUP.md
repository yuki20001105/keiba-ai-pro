...
1. **ブラウザをリロード** (Ctrl+R)
2. `/home` のヘッダーに表示される「管理者モード」をクリック
3. 現在ログイン中の管理者アカウントのパスワードを再入力
4. 同じ `/home` 内で管理者ワークスペースへ切り替わることを確認
5. 管理者ワークスペースから以下の機能へアクセス:
   - `/data-collection` - データ収集
   - `/train` - モデル学習
   - `/production-readiness` - 本番前チェック

一般ユーザーには「管理者モード」と管理者ワークスペースは表示されません。旧 `/admin` URL は管理画面を表示せず、ロックされた `/home` へリダイレクトされます。
