...
- Assessment is read-only and always reports `execution_performed=false`. Durable approval storage, approval-bound idempotent queued-job submission, service-only lease/fencing, and immutable private-bucket artifact registration now exist in the canonical bootstrap. UI submission wiring, deployed worker dispatch/training/upload, content attestation, evaluation comparison, and active-model switching remain unimplemented, so workflows #6 and #9 remain partial.
