...
- Assessment is read-only and always reports `execution_performed=false`. Durable approval storage and approval-bound idempotent queued-job submission now exist in the canonical bootstrap. UI submission wiring, worker lease/claim execution, artifact writing, evaluation comparison, and active-model switching remain unimplemented, so workflows #6 and #9 remain partial.
