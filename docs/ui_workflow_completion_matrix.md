...
- Assessment is read-only and always reports `execution_performed=false`. The Admin workbench can now create a pending request from the exact preview, load a shared approval ID, record an independent decision, let the original requester submit the approved job, and refresh job state. Durable approval/job storage, service-only lease/fencing, and immutable private-bucket artifact registration exist in the canonical bootstrap. Deployed worker dispatch/training/upload, content attestation, evaluation comparison, and active-model switching remain unimplemented, so workflows #6 and #9 remain partial.
