...
- Assessment is read-only and always reports `execution_performed=false`. Durable approval storage, approval-bound idempotent queued-job submission, and a service-only lease/fencing state contract now exist in the canonical bootstrap. UI submission wiring, deployed worker dispatch/training, artifact writing, evaluation comparison, and active-model switching remain unimplemented, so workflows #6 and #9 remain partial.
