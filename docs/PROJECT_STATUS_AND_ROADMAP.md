...
|---|---|---|
| Release contract split | `limited-production-observation-v1` separates `system_release_ready` from `model_business_validated`. The existing full READY path remains intact. Limited mode requires exact trusted system evidence and forces `MODEL_RUNTIME_STATUS=observation`, automatic betting off, deployed activation/training off, prospective `middle` observations, and rollback readiness. PR #28 is merged, trusted producer v4 is updated and locked, and the exact merge SHA is exercised in Staging | Assemble and independently review the sanitized limited-system observation, then exercise the protected limited evidence and release workflows |
...
| Multi-instance HA/fencing | Real Render Starter instances performed claim, SIGKILL, lease takeover, stale-fence rejection, and cleanup against Staging at `f5b9c90`. However, the retained report explicitly marks the exercised HA job `synthetic_contract_test=true` and `eligible_as_complete_phase3n_staging_observation=false`; it therefore proves hosted mechanics but cannot truthfully set the protected observation's `non_synthetic=true` field | Obtain explicit authorization for a new bounded two-instance exercise using a real approved Staging operation, or formally revise and reapprove the release contract. Do not relabel the existing synthetic workload |
...
| Trusted evidence | Trusted producer v4 is immutable at `c8308a3...`; all three GitHub Environments now admit v4 in addition to the historical v3 policy. The protected system-observation secret is still absent, so no workflow was dispatched. The operational Supabase database has 21 append-only history rows and no Preview Branch currently exists; it is deliberately ineligible for the fresh single-manifest bootstrap proof. Existing retained artifacts also do not yet establish non-synthetic rollback or the eleven hosted Auth/RLS/IDOR booleans for the complete envelope | For `limited-observation`, populate only `PHASE3N_STAGING_OBSERVATION_B64` after a fresh 21-migration disposable bootstrap, hosted Auth/RLS/IDOR smoke, timestamped cache rerun, non-synthetic HA and rollback drill, and exact provider IDs all pass. `MODEL_EVALUATION_OBSERVATIONS_GZIP_B64` remains deferred to full model validation after the observation period |
...
2. **In progress:** trusted producer v4 is updated, reviewed and locked; assemble the sanitized non-synthetic system observation and run `staging-evidence.yml` with `evidence_scope=limited-observation` for exact candidate `86a2d314...`.
