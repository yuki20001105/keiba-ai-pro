...
| Frontend tests | 31 test files and 329 tests passed | Verified |
...
| Python tests | Full `python-api/tests` suite contains 984 tests; 900 non-Phase3N-drift tests pass before commit, while the remaining 84 correctly fail closed on the modified canonical manifest until commit; the model builder/verifier slice passes 72 tests and feature consistency passes 73 | Full exact-commit result must be recorded after the queued-job ledger commit; remote exact-SHA CI remains pending |
...
| WP2 Finish operator workflow gaps | Harvester + Trainer + Oracle | WP1 for model decisions | **In progress:** quality bridge, authenticated Admin profiling viewer, read-only feature provenance/INV-01 catalog, strict retrain payload hashing, an Admin-only eligibility assessment, deployed-environment blocks on legacy model training/activation/deletion/repair, a private two-person approval ledger, and an approval-bound idempotent queued-job ledger are wired into the canonical bootstrap. Hosted application/runtime evidence, worker lease/claim and isolated artifact execution, separate switch/retirement approvals, standalone generation execution, and advanced evaluation remain | Accepted workflows meet G1; intentionally script-only items have runbooks | +6% |
...
| 2026-08-02 | `codex/fullstack-readiness` approval-bound job candidate | 66% authoritative / 68% provisional | NOT_READY | WP2 now atomically queues one actor/hash/CAS-bound durable job per independently approved retrain request, records append-only approval/job events, and keeps execution/artifact fields structurally false. The canonical bootstrap has 13 migrations; Frontend 329, 79-route authz, 71-route build, SQL parse, lint/typecheck, and scanners pass. Worker lease/claim, isolated artifact execution, hosted application, and exact-commit full Python remain pending. |
