...
> Evidence cutoff: hosted Staging, signed trusted evidence, Production migration, observation-only deployment, six-probe Production monitoring, and the first prospective Production settlement through 2026-08-23
> Current branch at assessment: `codex/record-production-observation-ops`, based on `origin/develop` after PR #44
...
> Candidate status: Limited Production has prospectively recorded 161 horse predictions across 11 real pre-start `middle` races under observational model candidate `a903259aa57057101fdebd6d1262d91d55d58208`, including 11 qualifying virtual bets. Two completed races have been settled append-only only after their official result pages exposed every finish and the win payout; formal progress is now 28/1,000 settled samples, 2/100 settled qualifying bets, and 1/90 settled observation days. A Production-local delete/rebuild check passes over all 161 prediction rows with digest `447e359c...c838` and `database_unchanged=true`. Automatic betting remains false. The market-free OOF winner-meta AUC remains 0.7581 against the approved 0.85 gate; the model remains observational, unvalidated, and unauthorized for activation
...
| Multi-instance HA/fencing | A fresh 21-migration Supabase Preview Branch and two Render Starter instances exercised the real `cache-rebuild` operation at exact candidate `86a2d314...`. Instance `fgmqt` was SIGKILLed while holding fence token 2; instance `skpvn` reclaimed token 3, rebuilt and verified the cache from four real Staging HA rows, applied one immutable effect, and rejected the stale token-2 apply. Render was then restored to its byte-identical pre-test environment, one Free instance, and healthy observation-only runtime; the Preview Branch was deleted. The sanitized report is `reports/evidence/phase3n/phase3n_non_synthetic_ha_rollback_20260816.json` | Bind this non-synthetic exercise into the complete protected system-observation envelope and have the trusted producer independently verify it |
| Observation collection | Limited Production recorded 161 predictions across 11 real pre-start `middle` races, with 11 qualifying virtual bets, zero retrospective predictions, and automatic betting disabled. All rows bind to observational model candidate `a903259...`; the provider runtime remains `6f705813...`. Races `202601020211` and `202604030208` were settled only after the official current result page contained every numeric finish and the win payout. The append-only RPC inserted 28 result events; source payload digests are `59782c3a...c21c` and `4ab33cad...d15c`. No prediction row was created or rewritten during settlement. Current formal progress is 28/1,000 settled samples, 2/100 settled qualifying bets, and 1/90 settled days | Keep the scheduled collector limited to current-JST-date, future-start, `middle` odds and settle only complete official results. Accumulate the remaining 972 samples, 98 qualifying bets, and 89 settled days; latency and business-value gates remain open |
...
| Cache integrity | The authorized non-paid local runtime read all 161 Production prediction rows from Supabase PostgreSQL, wrote and deleted only the local cache, rebuilt it from the authoritative rows, and verified matching before/after digest `447e359c...c838`, `database_unchanged=true`, and success. The report is bound to observational candidate `a903259...` at `reports/phase3n_production_cache_integrity_evidence.json` | Repeat after material ledger growth and bind the sanitized evidence into the later trusted model-evidence package; PostgreSQL remains authoritative |
...
| Limited Production runtime | PR #42 promoted the provider-bound monitor to `main` at `6f7058136ec99f41a8848757750f49e0ac5ff7a9`. Vercel Production is Ready and Render is Live at that SHA; both health endpoints remain observation-only with automatic betting false. PR #44 merged the fail-closed collector plus append-only settlement/cache commands after all 13 CI jobs and both Vercel checks passed. Manual collector run `32625167816` then passed on merge SHA `56d25925...`: it excluded all 11 already-observed races, found no remaining future candidate, wrote zero duplicates, uploaded sanitized evidence, and kept automatic betting false. The prior healthy Render deployment `dep-da57t6jbc2fs7384soog` at `83520c48428cce599fed2cbdf881b39c4116e3a8` remains the provider-bound rollback target | Continue scheduled prospective observations and complete official-result settlements without enabling betting, training, or activation |
...
The first qualifying prospective Production `middle` observation and official settlement are complete. The next operational milestone is a CI-green scheduled collector run followed by continued append-only settlement and periodic cache-integrity evidence refresh; this is evidence collection, not a blocker to the already-active Limited Production system release.

Model `validated`/`active` remains blocked by:

1. AUC 0.7581 versus 0.85 and insufficient prospective ROI/MaxDD evidence.
2. 28 settled valid samples, two settled qualifying bets, and one settled day versus 1,000/100/90.
...
| WP5 Run Staging security and model checks | Sysop + Trainer + Oracle | WP4 | **Partial:** hosted Auth/RLS/IDOR, approved staking/payout, exact-commit deployment, public health/OpenAPI 200, protected API 401, prospective Production `middle` observation, and the first complete official settlement pass. The model is not acceptance-complete | G2 security boundary and all approved model thresholds pass on real candidate data | +4% |
...
| WP8 Promote and observe Production | Sysop + Jobs + Ledger | WP7, release approval | **In progress:** controlled observation-only release, monitoring, rollback, 11 prospective races, 161 predictions, 11 qualifying virtual bets, 28 settled samples from two complete races, and a 161-row Production cache rebuild pass | G5 passes and the agreed 90-day/1,000-sample/100-bet observation period completes | +4% |
...
3. **Complete:** Production health/error/observation alerts, incident ownership, provider-bound rollback, and observation-only deployment pass with automatic betting disabled.
4. **Complete:** the protected limited release, exact provider deployment, smoke, and first prospective Production observations pass.
5. **Complete:** PR #44 merged the fail-closed scheduled collector and manual run `32625167816` proved safe idempotent exclusion of the 11 existing races. Continue official-result settlement and Production cache-integrity refresh while accumulating the remaining 972 settled samples, 98 qualifying bets, and 89 settled days.
...
| 2026-08-23 | First Production observation settlement and cache evidence | 78.15% authoritative / 78% reported; 85.87% system-release indicator | OBSERVATION_ACTIVE / MODEL_NOT_VALIDATED | Eleven prospective pre-start `middle` races produced 161 predictions and 11 qualifying virtual bets under observational candidate `a903259...`; automatic betting remained false. Official current-result pages supplied complete finish and win-payout data for `202601020211` and `202604030208`, so append-only settlement inserted 28 result events with source digests `59782c3a...c21c` and `4ab33cad...d15c`. Other unfinished races failed closed without a write. The Production-local cache delete/rebuild verified all 161 prediction rows with digest `447e359c...c838` and `database_unchanged=true`. Full Python tests pass 1,251; PR #44's remote CI passed all 13 jobs and both Vercel checks. Post-merge collector run `32625167816` safely excluded all 11 existing races and performed no duplicate write. Formal business progress is 1/90 days, 28/1,000 settled samples, and 2/100 settled qualifying bets; AUC/ROI and trusted model evidence remain open. |
