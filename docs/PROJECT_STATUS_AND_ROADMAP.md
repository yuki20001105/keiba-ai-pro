...
> Candidate status: Limited Production has prospectively recorded 161 horse predictions across 11 real pre-start `middle` races under observational model candidate `a903259aa57057101fdebd6d1262d91d55d58208`, including 11 qualifying virtual bets. The first race, `202601020211`, was settled append-only only after its official result page exposed all 16 finish positions and the win payout; formal progress is now 16/1,000 settled samples, 1/100 settled qualifying bets, and 1/90 settled observation days. A Production-local delete/rebuild check passes over all 161 prediction rows with digest `447e359c...c838` and `database_unchanged=true`. Automatic betting remains false. The market-free OOF winner-meta AUC remains 0.7581 against the approved 0.85 gate; the model remains observational, unvalidated, and unauthorized for activation
...
| Observation collection | Limited Production recorded 161 predictions across 11 real pre-start `middle` races, with 11 qualifying virtual bets, zero retrospective predictions, and automatic betting disabled. All rows bind to observational model candidate `a903259...`; the provider runtime remains `6f705813...`. Race `202601020211` was settled only after the official current result page contained all 16 numeric finishes and the win payout. The append-only RPC inserted 16 result events with source payload digest `59782c3a...c21c`; no prediction row was created or rewritten during settlement. Current formal progress is 16/1,000 settled samples, 1/100 settled qualifying bets, and 1/90 settled days | Keep the scheduled collector limited to current-JST-date, future-start, `middle` odds and settle only complete official results. Accumulate the remaining 984 samples, 99 qualifying bets, and 89 settled days; latency and business-value gates remain open |
...
2. 16 settled valid samples, one settled qualifying bet, and one settled day versus 1,000/100/90.
...
| WP8 Promote and observe Production | Sysop + Jobs + Ledger | WP7, release approval | **In progress:** controlled observation-only release, monitoring, rollback, 11 prospective races, 161 predictions, 11 qualifying virtual bets, one complete settlement, and a 161-row Production cache rebuild pass | G5 passes and the agreed 90-day/1,000-sample/100-bet observation period completes | +4% |
...
| 2026-08-23 | First Production observation settlement and cache evidence | 78.15% authoritative / 78% reported; 85.87% system-release indicator | OBSERVATION_ACTIVE / MODEL_NOT_VALIDATED | Eleven prospective pre-start `middle` races produced 161 predictions and 11 qualifying virtual bets under observational candidate `a903259...`; automatic betting remained false. After race `202601020211` finished, the official current-result page supplied all 16 numeric finishes and the win payout. Append-only settlement inserted 16 result events with source digest `59782c3a...c21c`. The Production-local cache delete/rebuild then verified all 161 prediction rows with digest `447e359c...c838` and `database_unchanged=true`. Full Python tests pass 1,251; PR #44's remote CI passed all 13 jobs and both Vercel checks. Post-merge collector run `32625167816` safely excluded all 11 existing races and performed no duplicate write. Formal business progress is 1/90 days, 16/1,000 settled samples, and 1/100 settled qualifying bets; AUC/ROI and trusted model evidence remain open. |
