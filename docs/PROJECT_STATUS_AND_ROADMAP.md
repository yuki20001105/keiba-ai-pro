...
> Candidate status: approval binding, exact-commit CI, append-only 21-migration verification, and exact-commit Staging regression pass; observation capture is enabled but the real ledger still has zero rows, the current model fails strict annual speed-deviation walk-forward AUC/ROI acceptance, and the new licensed-history/OOF value path remains research-only, unapproved, and undeployed
...
| Speed-deviation annual walk-forward | A non-activating LightGBM regression pipeline predicts realized speed deviation directly and now uses strict outer-year folds for 2016, 2017, 2018, and 2025. Each fold fits its distance/surface target baseline on pre-fold outcomes only, chooses tree count and probability temperature inside the training period, fixes the approved `phase3n-tansho-flat-v1` policy before outer evaluation, and has zero future-field intersection. Across 264,705 settled entries and 23,208 races it records RMSE 0.6069, Spearman 0.8025, calibrated winner AUC 0.8047, Brier 0.0694, and ECE 0.0045. AUC remains below 0.85; policy-correct ROI is -23.23%, drawdown exceeds 20%, and ROI delta to the favorite baseline is -2.76 points. The earlier forced-one-wager-per-race ROI is superseded because it did not enforce the approved minimum expected value of 1.2 | Do not activate or deploy this candidate. Acquire licensed or otherwise explicitly authorized machine-readable 2019-2024 history, rebuild continuous annual folds, improve the speed-score-to-win-probability/value layer using training-only selection, and reserve all results after 2026-07-11 for final unknown/prospective evidence |
| Licensed history and OOF value path | A transactional append-only importer now validates provider/license references, complete races, file SHA-256, 2019-2024 scope, required settled fields, and fresh pre-decision odds before writing a separate provenance table. The loader gives an overlapping licensed canonical runner priority without deleting legacy bytes. A single point-in-time odds contract, expanding-year OOF winner-probability meta-model, and inner-meta-OOF-only EV/edge/odds-band/bet-rate search are implemented. Every produced value artifact is hard-marked candidate-only, unapproved, inactive, undeployed, and not deployment-eligible. The current database audit still finds zero licensed rows and zero timestamped odds rows; JRA-VAN/JV-Link software or an authorized export was not found locally | Operator exports authorized 2019-2024 machine-readable history with timestamped pre-race odds, validates it by dry run, then explicitly applies it to a recoverable research DB copy. Only after all six years pass the readiness audit may continuous base-model OOF generation and nested outer evaluation run |
...
The shortest remaining route is now two parallel tracks. Model track: obtain the authorized 2019-2024 export -> validate hashes, complete races, provenance, and pre-race odds timestamps -> append it to a recoverable research DB copy -> require all six audit years -> generate expanding-year base speed OOF predictions -> fit the OOF winner meta-model -> select EV/edge/odds-band/bet-rate conditions on inner meta-OOF only -> freeze the selected numeric condition -> evaluate untouched later outer folds -> seek a new durable policy approval only if every historical gate passes. Operations track: capture the first real exact-commit observation after odds publication -> run hosted cache-integrity evidence from an authorized runtime -> continue append-only prospective observations. Only a candidate that passes the fast historical screen should consume the full approved 90-day/1,000-sample/100-bet prospective window. Neither retrospective replay, synthetic data, nor fabricated wagers may be registered as prospective Staging evidence.
