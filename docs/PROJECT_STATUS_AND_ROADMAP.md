...
> Current implementation checkpoint: `9665c8c` (`codex/fullstack-readiness` local candidate)
...
|---|---|---|
| Git source | Candidate branch `codex/fullstack-readiness` remains based on `origin/develop` commit `d9bbcbc` and is 39 local commits ahead at checkpoint `9665c8c`; all readiness work is local | Correct base; push and exact-SHA CI evidence still require authorization |
...
| Python tests | Full `python-api/tests` suite passes 1,137 tests on exact commit `9665c8c`; the model builder/verifier slice passes 72 tests, the source-readiness audit passes 8, and feature consistency passes 73 | Locally verified on worktree Python 3.11.9; remote exact-SHA CI remains pending |
...
| Model acceptance path | Strict row observations are recomputed into digest-bound evidence; a read-only source audit now distinguishes usable evidence fields from legacy prediction records | Parent DB audit found 1,014 same-day post-cutoff active-model prediction rows but only 15 settled labels and none of the eight required timezone/freshness/settlement/wager/baseline/latency capabilities; no approved thresholds or trusted run exists |
...
| WP2 Finish operator workflow gaps | Harvester + Trainer + Oracle | WP1 for model decisions | **In progress:** quality bridge, authenticated Admin profiling viewer, read-only feature provenance/INV-01 catalog, strict retrain payload hashing, an Admin-only eligibility assessment, deployed-environment blocks on legacy model training/activation/deletion/repair, an Admin request/independent-decision/job-status panel, private two-person approval/job ledgers, service-only CAS/lease/fencing transitions, immutable private-bucket artifact/evaluation registration, a service-only execution bundle, an isolated OOT trainer, and a fail-closed one-shot coordinator are implemented. A bounded one-shot dispatcher now selects at most five exact policy/commit/active-model candidates through a read-only service projection, resolves digest-named immutable snapshots, independently revalidates all bindings, and delegates each claim to the fenced coordinator. A separate one-shot reconciler recovers expired leases, exposes only old terminal-failed/unregistered exact-name objects, deletes through private Storage, and records deleted/not-found/failed/no-op outcomes in an immutable service-only ledger. Deployment and recurring scheduling, hosted PostgreSQL/Storage runtime evidence, a trusted evaluator and candidate comparison, separate switch/retirement approvals, standalone generation execution, and advanced evaluation remain | Accepted workflows meet G1; intentionally script-only items have runbooks | +6% |
