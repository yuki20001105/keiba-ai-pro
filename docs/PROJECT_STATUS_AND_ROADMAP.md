...
| Repository gates | The current local full Python suite passes **1,296** tests with 6 skips; secret and test-weakening scanners report zero findings. The last deployed runtime commit `961a5be` passes all 13 jobs in [CI run 31815636708](https://github.com/yuki20001105/keiba-ai-pro/actions/runs/31815636708). Remote CI for the official-history research changes is still pending | Push the research checkpoint, require all remote jobs to pass, and preserve the mixed-generation time/label coalescing regressions |
...
The shortest remaining route is now two parallel tracks. Model track: use the completed official outcome history to improve leakage-safe pre-race features -> rerun expanding annual speed folds until the AUC ability gate passes -> generate base speed OOF predictions -> fit the winner meta-model. Result-PDF final odds remain quarantined, so nested EV search and ROI/DD acceptance wait for genuine timestamped quotes. Operations track: capture the first real exact-commit observation after odds publication -> run hosted cache-integrity evidence from an authorized runtime -> continue append-only prospective observations. Only a candidate that passes the ability screen should consume the full approved 90-day/1,000-sample/100-bet prospective window. Neither retrospective replay, synthetic data, final-result odds, nor fabricated wagers may be registered as prospective Staging evidence.
