...
> Candidate status: approval binding, exact-commit CI, append-only 21-migration verification, and exact-commit Staging regression pass; observation capture is enabled but the real ledger still has zero rows, and the current model fails strict annual speed-deviation walk-forward AUC/ROI acceptance
