...
> Current runtime implementation checkpoint: `fc56d2210993d27d670d88b7fdf3c68069e5821c`, which is CI-green, Live on Render Staging, and bound to the same exact Phase 3N candidate SHA
> Status: **overall 73.15%, reported as 73% (reasonable range: 71-76%), Production NOT_READY**
> Candidate status: approval binding, exact-commit CI, append-only 21-migration verification, and exact-commit Staging regression pass; observation capture is enabled but the real ledger still has zero rows. The stale live-input path is fixed and Live at exact `fc56d22`; manual health/auth/actual-market checks pass, while `yoso` remains correctly excluded. The free JRA official-history path is complete for research outcomes. A market-free, strictly shifted prior-speed and inner-period OOF winner-meta evaluation improves 2020-2024 AUC from 0.7137 to 0.7581, but still fails 0.85 and has no point-in-time odds for ROI; all generated candidates remain research-only, unapproved, and undeployed
...
|---|---|---|
| Isolated Staging | Vercel, Render, and Supabase are isolated. Supabase preserves the original 19 rows and has the approved append-only ordinals 20-21; Render is one Free instance and is Live at `fc56d2210993d27d670d88b7fdf3c68069e5821c`, with `PHASE3N_CANDIDATE_COMMIT_SHA` aligned to the same exact SHA | Preserve the deployed runtime identity and append-only history through merge and evidence collection |
...
| Observation collection | Exact deployed checkpoint `fc56d22` is aligned with the Render candidate-SHA boundary, Staging project, expanding-window assertion, and enabled capture. It removes the bodyless resume short circuit, detects UTF-8/EUC-JP, force-refreshes mutable race pages, rejects the legacy race-id date slice, atomically replaces the complete race snapshot, and reads actual dynamic odds. Manual hosted regression returns health 200 and unauthenticated analyze 401. The 2026-08-16 candidate page supplies date/distance/15 runners but remains `yoso`; it is correctly excluded and no observation is written. The last read-only ledger check remains zero prediction/result observations | Establish the first prospective observation only when an actual `middle` market snapshot exists before the result. Then run cache integrity and allow the 90-day/1,000-sample/100-bet window without synthetic, forecast, final-result, or retrospective backfill |
...
| Repository gates | The live-input checkpoint `fc56d22` passes all 13 jobs in [CI run 31878652117](https://github.com/yuki20001105/keiba-ai-pro/actions/runs/31878652117), is Live on Render, and passes hosted health/auth regression. The full local baseline at that checkpoint passed 1,305 Python tests with 6 skips and 335 Frontend tests. The new market-exclusion, OOF winner-meta, and shifted speed-history policy tests pass locally | Run the full current-commit suites and exact-commit CI before treating the new research implementation as mergeable; do not deploy a model from a historical development report |
...
The shortest remaining route is now two parallel tracks. Model track: preserve the new market-free prior-speed/OOF-meta baseline -> add only richer legitimate pre-race context -> evaluate on a newly reserved period rather than retuning the observed 2020-2024 folds -> require the AUC gate before activation. Result-PDF final odds remain quarantined, so nested EV search and ROI/DD acceptance wait for genuine timestamped quotes. Operations track: the exact-SHA live-input remediation and hosted health/auth/input regression are complete -> capture the first real observation only after an actual `middle` snapshot -> run hosted cache-integrity evidence from an authorized runtime -> continue append-only prospective observations. Only a candidate that passes the ability screen should consume the full approved 90-day/1,000-sample/100-bet prospective window. Neither retrospective replay, synthetic data, forecast odds, final-result odds, nor fabricated wagers may be registered as prospective Staging evidence.
