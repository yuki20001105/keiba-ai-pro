...
> Candidate status: approval binding, exact-commit CI, append-only 21-migration verification, and exact-commit Staging regression pass; observation capture is enabled but the real ledger still has zero rows. The first real-race attempt exposed and safely rejected a stale live-input path; the local remediation now passes 1,305 Python tests but is not yet deployed or prospective evidence. The free JRA official-history path is complete for research outcomes, but the 2020-2024 speed-deviation walk-forward fails AUC and has no point-in-time odds for ROI; all generated candidates remain research-only, unapproved, and undeployed
...
| Observation collection | Exact deployed checkpoint `961a5be` is aligned with the Render candidate-SHA boundary, Staging project, expanding-window assertion, and enabled capture. A read-only database check still reports zero prediction and zero result observations. After actual odds were visible for the 2026-08-15 race, Render still reused an earlier incomplete snapshot (`distance=0`, odds/popularity missing); Q1 correctly returned 400 before capture. Local remediation removes the bodyless resume short-circuit, detects UTF-8/EUC-JP, force-refreshes mutable race pages, rejects the legacy race-id date slice, atomically replaces the complete race snapshot, and reads actual dynamic odds while excluding `yoso` forecast odds. A read-only 2026-08-16 page probe recovered date, distance, and all 15 runners; its forecast odds remained excluded as required | Push and deploy the remediation at an exact CI-green SHA, repeat health/auth/input regression, then establish the first prospective observation only when actual market odds are available. Allow the 90-day/1,000-sample/100-bet window to run without synthetic or post-result backfill |
...
| Speed-deviation annual walk-forward | The free official-history evaluation now uses strict outer years 2020-2024. Each fold fits its distance/surface baseline, tree count, and probability temperature using pre-fold data only. Current-race outcome fields and market fields (`odds`, `popularity`, quote metadata, and their derivatives) are excluded. Across 229,190 entries and 16,644 races it records RMSE 2.0213, Spearman 0.6526, winner AUC 0.7137, Brier 0.06455, and ECE 0.00545. Calibration and sample count pass, but AUC remains below 0.85. All ROI/DD/bet gates fail closed because no genuine point-in-time historical odds exist. The 2016/2017/2018/2025 screen remains historical context only | Do not activate or deploy this candidate. Improve pre-race ability features using only information available before post time, rerun expanding folds, then build the OOF winner meta-model. Keep ROI/nested purchase search blocked until genuine timestamped odds are collected |
| Official history and OOF value path | The free/no-contract outcome route is complete. The official-host-only index contains 1,729 unique JRA PDFs for 2019-2024; all exact bytes have URL, UTC retrieval time, size, and SHA-256. Conversion produced 273,187 timed runners in 19,846 flat races. It audited 755 jump races and explicitly excluded 87 unreadable/incomplete flat-race layouts plus three unsupported-font source documents rather than guessing. Full-bundle dry-run validated all 273,187 records without writing, then the identical manifest `4e52d5f...d399e` was appended to a recoverable research DB copy. `PRAGMA quick_check` is `ok`; annual authorized outcome coverage is 3,281-3,325 races and passes the speed-fold readiness gate. Result-time odds remain quarantined outside the training payload | Preserve this immutable research evidence and its reviewed exclusions. Improve the ability feature layer; collect genuine point-in-time odds prospectively in Staging. Only those quotes may unlock OOF/nested ROI evaluation |
...
| Repository gates | The current local full Python suite passes **1,305** tests with 6 skips, including nine live-input refresh regressions; secret and test-weakening scanners report zero findings. The full Frontend suite also passes 335 tests; its existing React `act(...)` warnings remain non-blocking. The last deployed runtime commit `961a5be` passes all 13 jobs in [CI run 31815636708](https://github.com/yuki20001105/keiba-ai-pro/actions/runs/31815636708). Remote CI for the live-input remediation is pending | Push the remediation checkpoint, require all remote jobs to pass, deploy only that exact SHA to isolated Staging, and preserve the mixed-generation time/label coalescing and live-input freshness regressions |
...
The shortest remaining route is now two parallel tracks. Model track: use the completed official outcome history to improve leakage-safe pre-race features -> rerun expanding annual speed folds until the AUC ability gate passes -> generate base speed OOF predictions -> fit the winner meta-model. Result-PDF final odds remain quarantined, so nested EV search and ROI/DD acceptance wait for genuine timestamped quotes. Operations track: make the live-input remediation CI-green -> deploy its exact SHA to isolated Render Staging -> repeat health/auth/input regression -> capture the first real observation only after actual odds publication -> run hosted cache-integrity evidence from an authorized runtime -> continue append-only prospective observations. Only a candidate that passes the ability screen should consume the full approved 90-day/1,000-sample/100-bet prospective window. Neither retrospective replay, synthetic data, forecast odds, final-result odds, nor fabricated wagers may be registered as prospective Staging evidence.
...
| 2026-08-15 | Free JRA official-history full evaluation | 73.15% authoritative / 73% reported | NOT_READY | All 1,729 official 2019-2024 PDFs were hash-recorded and converted into 273,187 timed runners across 19,846 flat races; 755 jump races, 87 unreadable/incomplete flat layouts, and three unsupported-font documents are explicitly audited rather than guessed. The identical manifest passed a no-write 273,187-record dry-run and append-only import to a research DB copy; `quick_check=ok` and every year exceeds 3,281 outcome races. The corrected 2020-2024 expanding evaluation excludes future and market fields and covers 229,190 entries / 16,644 races. AUC 0.7137 fails 0.85; Brier 0.06455 and ECE 0.00545 pass; ROI/DD remain unavailable because point-in-time odds are absent. No model was approved, activated, or deployed. |
