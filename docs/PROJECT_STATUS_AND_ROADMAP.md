...
| Repository gates | The live-input checkpoint `fc56d22` is Live on Render and passes hosted health/auth regression. Nested-value implementation `3d24948` passes all 13 jobs in [CI run 31884370561](https://github.com/yuki20001105/keiba-ai-pro/actions/runs/31884370561), including Python, Frontend, Playwright, both containers, dependency/security, and Phase3N HA. Both Vercel checks also pass. Local validation passes 1,196 FastAPI tests plus 118 research tests with 6 skips; secret and test-weakening scanners both report zero findings | Review and merge the research evaluator without deploying a candidate model; then wait for genuine prospective `middle` rows before running or accepting ROI evaluation |
