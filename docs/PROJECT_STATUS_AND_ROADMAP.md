...
> Current implementation checkpoint: `0b7ec25` (`codex/fullstack-readiness` local candidate)
...
| Python tests | Full `python-api/tests` suite passes 1,029 tests on exact commit `0b7ec25`; the model builder/verifier slice passes 72 tests and feature consistency passes 73 | Locally verified on worktree Python 3.11.9; remote exact-SHA CI remains pending |
...
| WP2 Finish operator workflow gaps | Harvester + Trainer + Oracle | WP1 for model decisions | **In progress:** quality bridge, authenticated Admin profiling viewer, read-only feature provenance/INV-01 catalog, strict retrain payload hashing, an Admin-only eligibility assessment, deployed-environment blocks on legacy model training/activation/deletion/repair, an Admin request/independent-decision/job-status panel, a private two-person approval ledger, an approval-bound idempotent job ledger, service-only CAS/lease/fencing worker transitions, immutable private-bucket artifact/evaluation registration, a service-only execution-bundle projection, and a locally isolated trainer that pins snapshot/features/commit/periods, fits preprocessing only on the approved train window, and reserves validation for OOT evaluation. A deployed dispatcher, heartbeat supervisor, snapshot materializer, uploader/evaluator, hosted runtime evidence, trusted attestation and candidate comparison, separate switch/retirement approvals, standalone generation execution, and advanced evaluation remain | Accepted workflows meet G1; intentionally script-only items have runbooks | +6% |
