...
ログイン後、ホーム画面には日常操作に必要な2つの基本ステップが表示されます。データ取得、本番前チェック、モデル管理は、同じホーム画面内の管理者モードへ集約しています。一般ユーザーには管理者モードの入口も管理機能も表示しません。
...
1. 予測実行: 日付とモデルを指定して対象レースを表示します。
2. 成績確認: virtual betや予測結果の成績を確認します。
...
1. Adminロールでログインし、ホーム右上の「管理者モード」を押します。
2. 現在ログインしているアカウントのパスワードを入力します。
3. 「確認して切り替える」を押します。
4. URLが`/home`のまま、黒基調の管理者モードへ切り替わることを確認します。

管理者モードは15分で自動解除されます。同じログインセッションなら再読み込み後も残り時間だけ復元され、ログアウトまたは「ユーザー画面に戻る」で直ちに解除されます。パスワードは画面状態やブラウザStorageへ保存しません。旧`/admin`を直接開いた場合は、管理モードを解除してホームへ戻ります。

`/data-collection`、`/train`、`/production-readiness`は有効な管理者モードからだけ表示できます。一般ユーザーまたはロック中のAdminがURLを直接開いた場合も、管理内容をマウントせず`/home`へ戻ります。
...
通常画面には、期間指定、Dry-run、通常取得、進捗・状態不明時の安全確認、最新のfetch summary 1件、取得済みデータ3指標だけを表示します。修復・再取得計画、限定ライブ検証、監査キュー、プロファイリングなどの保守機能は削除していませんが、誤操作を避けるため通常画面からは非表示です。
...
2. ログイン後、ホーム右上の「管理者モード」でパスワードを再確認し、「データ取得」を開きます。
...
7. 完了後、最新のfetch summaryと「取得済みデータ」の総レース数・総出走馬数・最終取得日を確認します。
...
別の取得が進行中の場合、画面には対象期間・受付または開始時刻・job IDを表示し、完了までDry-runと取得開始を無効にします。状態は自動再確認されます。60秒を超えるDry-runはユーザー別のブラウザ保存領域にjob IDを保持し、画面再読込後も同じjobを確認します。

現在は開始済みジョブを個別に安全終了する機能はありません。`stop-keiba-ai-pro.bat`やブラウザ終了はプロセス／監視の停止であり、ジョブのキャンセルではありません。未完了ジョブは次回起動時に再開されるため、ジョブDBの直接編集やPythonプロセスだけの強制終了は行わず、完了を待ってください。
...
管理者メニューの「本番前チェック」からbuild、health、smoke、flag確認を実行できます。業務DBのwrite系APIやモデルActivationは実行しませんが、ローカルにはbuild・compile由来の生成物が作成される場合があります。
